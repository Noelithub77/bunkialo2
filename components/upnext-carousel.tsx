...
import { formatTimeDisplay, getDayName, getNearbySlots } from '@/stores/timetable-store'
...
export function UpNextCarousel({ slots }: UpNextCarouselProps) {
    ...
    const resetSnapTimer = useCallback(() => {
        ...
        const centerIndex = Math.min(2, nearbySlots.length - 1)
    ...
    const renderCard = ({ item, index }: { item: TimetableSlot; index: number }) => {
        const courseColor = getCourseColor(item.courseId)
        const isCurrentlyActive = item.dayOfWeek === currentDay &&
            ...
            <View style={[styles.cardWrapper, { width: CARD_WIDTH }]}>
                <LinearGradient
                    ...
                    style={[
                        ...
                        isCurrentlyActive && { borderColor: Colors.status.success, borderWidth: 2 },
                    ]}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 1 }}
                >
                    ...
                    <View style={styles.statusRow}>
                        <View style={[
                            styles.statusDot,
                            { backgroundColor: isCurrentlyActive ? Colors.status.success : courseColor }
                        ]} />
                        <Text style={[styles.statusText, { color: isCurrentlyActive ? Colors.status.success : theme.textSecondary }]}>
                            {isCurrentlyActive ? 'Now' : dayLabel}
                        </Text>
                    </View>
