...
export function CurrentClassCard({ currentClass, nextClass }: CurrentClassCardProps) {
    ...
    useEffect(() => {
        ...
        const updateTimer = () => {
            const now = new Date()
            const [hours, minutes] = (isCurrentlyActive ? displayClass.endTime : displayClass.startTime).split(':').map(Number)
    ...
    return (
        <LinearGradient colors={gradientColors} style={styles.card} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
            ...
            <View style={styles.statusRow}>
                <View style={[styles.statusDot, { backgroundColor: isCurrentlyActive ? Colors.status.success : Colors.status.warning }]} />
                <Text style={[styles.statusText, { color: isCurrentlyActive ? Colors.status.success : Colors.status.warning }]}>
                    {isCurrentlyActive ? 'Now' : 'Next'}
                </Text>
                {!isCurrentlyActive && displayClass.dayOfWeek !== new Date().getDay() && (
            ...
            <View style={styles.sessionTypeRow}>
                <View style={[styles.sessionTypeBadge, { backgroundColor: theme.backgroundSecondary }]}>
                    <Text style={[styles.sessionTypeText, { color: theme.textSecondary }]}>
                        {displayClass.sessionType.charAt(0).toUpperCase() + displayClass.sessionType.slice(1)}
                    </Text>
                </View>
            </View>
        </LinearGradient>
