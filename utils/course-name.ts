...
const extractCourseName = (courseName: string): string => {
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    console.log(`[COURSE_NAME] Pattern ${i} test result:`, pattern.test(trimmed))
    
    if (pattern.test(trimmed)) {
      const result = trimmed.replace(pattern, '').trim()
      console.log(`[COURSE_NAME] Pattern ${i} MATCHED! Result:`, JSON.stringify(result))
      return result
    }
  }
  
  console.log('[COURSE_NAME] No pattern matched, returning original:', JSON.stringify(trimmed))
  return trimmed
}
