...
Item {
  ...
  function maybeStartRefresh() {
    if (!root.pendingRefresh || root.wifixChecking) return
    root.pendingRefresh = false
    if (Wifix.isCampusSsid(root.currentSsid) && !root.wifixConnected) {
      root.setError("Campus portal detected · connect WiFi before refreshing")
      return
    }
  ...
  function startFetch() {
    ...
    fetchProc.command = ["curl", "-4", "-sSL", "--fail-with-body", "--retry", "2",
      "--retry-delay", "1", "--retry-all-errors", "--max-time", "20", "-K", "-"]
    fetchProc.running = true
  }

  function applySnapshot(raw) {
    var response = String(raw || "").trim()
    try {
      fetchProc.fetchHandled = true
      console.log("[Bunkialo] snapshot response received: bytes=" + response.length)
      if (response === "") {
        root.setError("Refresh failed · empty response · cached data kept")
        return
      }
      if (response.charAt(0) === "<") {
        root.setError("Refresh blocked by campus portal · cached data kept")
        return
      }
      var parsed = JSON.parse(response)
      if (parsed && typeof parsed === "object" && parsed.snapshot
        && typeof parsed.snapshot === "object" && !Array.isArray(parsed.snapshot)) {
        console.log("[Bunkialo] unwrapping compatible snapshot response")
        parsed = parsed.snapshot
      }
      if (parsed && typeof parsed.error === "string" && parsed.error !== "") {
        console.log("[Bunkialo] Worker returned an error: " + parsed.error)
        root.setError(parsed.error)
        return
      }
      var next = Api.parseSnapshot(JSON.stringify(parsed))
      console.log("[Bunkialo] snapshot accepted: timetable=" + next.timetable.length
        + " notifications=" + next.notifications.length)
      ...
      var reason = error && error.message ? String(error.message) : String(error || "unknown error")
      console.log("[Bunkialo] snapshot rejected: bytes=" + response.length + " reason=" + reason)
  ...
  Process {
    ...
    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: fetchProc.lastError = Wifix.processError(text)
    }
    onStarted: {
      fetchProc.fetchHandled = false
      fetchProc.lastError = ""
      console.log("[Bunkialo] snapshot request started: endpoint=" + root.apiOrigin + "/api/desktop/snapshot")
      fetchProc.write(Wifix.curlConfig({
        url: root.apiOrigin + "/api/desktop/snapshot",
        header: "Authorization: Credentials " + root.token,
        "retry": "2",
        "retry-delay": "1"
      }))
    ...
    onExited: function(exitCode) {
      console.log("[Bunkialo] snapshot request exited: curl_exit=" + exitCode
        + " response=" + fetchProc.fetchHandled + " stderr=" + fetchProc.lastError)
      if (exitCode !== 0 && !fetchProc.fetchHandled)
        root.setError("Refresh failed · cached data kept")
    }
  }
