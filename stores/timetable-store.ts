...
import type { DayOfWeek, SessionType, TimetableSlot, TimetableState } from '@/types'
...
const parseSlotTime = (dateStr: string): { dayOfWeek: DayOfWeek; startTime: string; endTime: string } | null => {
...
export const getRandomCourseColor = (): string => {
    return Colors.courseColors[Math.floor(Math.random() * Colors.courseColors.length)]
}
...
export const getCurrentAndNextClass = (slots: TimetableSlot[], now: Date = new Date()) => {
