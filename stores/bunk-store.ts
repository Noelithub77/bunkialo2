  ...
  persist(
    (set, get) => ({
      ...
      syncFromLms: () => {
        ...
        const updatedCourses: CourseBunkData[] = attendanceCourses.map(course => {
          const existing = currentBunks.find(c => c.courseId === course.courseId)
          ...
          if (existing) {
            ...
            const userBunks = existing.bunks.filter(b => b.source === 'user')
      ...
      resetToLms: () => {
        ...
        const freshCourses: CourseBunkData[] = attendanceCourses.map(course => {
          const lmsBunks: BunkRecord[] = course.records
            .filter(r => r.status === 'Absent')
            .map(r => ({
              id: generateId(),
      ...
      addBunk: (courseId, bunk) => {
        ...
        set(state => ({
          courses: state.courses.map(c =>
            c.courseId === courseId ? { ...c, bunks: [...c.bunks, newBunk] } : c
          ),
        }))
      },
      ...
      updateBunkNote: (courseId, bunkId, note) => {
        set(state => ({
          courses: state.courses.map(c =>
            c.courseId === courseId
              ? { ...c, bunks: c.bunks.map(b => (b.id === bunkId ? { ...b, note } : b)) }
              : c
          ),
        }))
      },
      ...
      markAsDutyLeave: (courseId, bunkId, note) => {
        set(state => ({
          courses: state.courses.map(c =>
            c.courseId === courseId
              ? { ...c, bunks: c.bunks.map(b => (b.id === bunkId ? { ...b, isDutyLeave: true, dutyLeaveNote: note } : b)) }
              : c
          ),
        }))
      },
      ...
      removeDutyLeave: (courseId, bunkId) => {
        set(state => ({
          courses: state.courses.map(c =>
            c.courseId === courseId
              ? { ...c, bunks: c.bunks.map(b => (b.id === bunkId ? { ...b, isDutyLeave: false, dutyLeaveNote: '' } : b)) }
              : c
          ),
        }))
      },
...
export const selectAllDutyLeaves = (courses: CourseBunkData[]): DutyLeaveInfo[] => {
